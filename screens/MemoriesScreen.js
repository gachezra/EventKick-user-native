...
export default function MemoriesScreen({ navigation }) {
  ...
  const fetchRegisteredEvents = async () => {
    try {
      ...
      toast.error('Error fetching registedred events, reload app!')
    }
  };
  ...
  const uploadAlbum = async (media, eventId) => {
    ...
    try {
      for (const item of media) {
        ...
        const fileKey = `eventkick/${user._id}/${eventId}/${fileName}`;
  ...
  const renderAlbum = ({ item }) => {
    ...
    return (
      <View style={styles.albumContainer}>
        <TouchableOpacity 
          style={styles.album}
          onPress={() => navigation.navigate('Pics', { allMedia: media, eventTitle: item.title, eventId: item._id })}
        >
          {coverMedia ? (
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
          ...
          <TouchableOpacity
            activeOpacity={1}
            onPress={()=>navigation.navigate('EventMedia')}
            >
            <Text style={styles.title}>From Events</Text>
          </TouchableOpacity>
        </View>
        <FlatList
          data={events}
